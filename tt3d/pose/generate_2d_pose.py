"""
Generates the 2D and 3D pose of the players from a video
"""
import logging
import mimetypes
...
def main():
    ...
    mmpose_dir = Path("/home/gossard/Code/tt_pose/mmpose")
    tt_pose_dir = Path("/home/gossard/Code/tt_pose/tt_pose")
    # Detection
    det_config = mmpose_dir / "demo/mmdetection_cfg/rtmdet_m_640-8xb32_coco-person.py"
    det_checkpoint = (
        tt_pose_dir / "models" / "rtmdet_m_8xb32-100e_coco-obj365-person-235e8209.pth"
    )
    # 2D pose estimator
    ...
    pose_lifter_config = (
        mmpose_dir
        / "configs/body_3d_keypoint/motionbert/h36m/motionbert_dstformer-ft-243frm_8xb32-120e_h36m-original.py"
    )
    pose_lifter_checkpoint = (
        tt_pose_dir / "models" / "motionbert_ft_h36m-d80af323_20230531.pth"
    )
    ...
    detector = init_detector(
        det_config, str(det_checkpoint), device=args.device.lower()
    )
    detector.cfg = adapt_mmdet_pipeline(detector.cfg)

    pose_estimator = init_model(
        pose_estimator_config,
        str(pose_estimator_checkpoint),
    ...
    pose_lifter = init_model(
        pose_lifter_config, str(pose_lifter_checkpoint), device=args.device.lower()
    )
    ...
    if args.output_root == "":
        ...
        output_file = os.path.join(args.output_root, os.path.basename("rtmpose.mp4"))
    ...
    if args.save_predictions:
        ...
        args.pred_save_path_3d = (
            f"{args.output_root}/results_"
            f"{os.path.splitext(os.path.basename(args.input))[0]}_3d.json"
        )
        args.pred_save_path_2d = (
            f"{args.output_root}/results_"
            f"{os.path.splitext(os.path.basename(args.input))[0]}_2d.json"
        )
    ...
    while video.isOpened():
        ...
        if args.save_predictions:
            ...
            pred_instances_list.append(
                dict(frame_id=frame_idx, instances=split_instances(pred_3d_instances))
            )
            pred_2d_instances_list.append(
                dict(frame_id=frame_idx, instances=pose_est_results_list[-1][:])
            )
    ...
    if args.save_predictions:
        ### Saving 2D data
        with open(args.pred_save_path_2d, "w") as f:
            json.dump(
                dict(
                    meta_info=pose_estimator.dataset_meta,
                    instance_info=pred_2d_instances_list,
                ),
                f,
                indent=4,
            )
        #### Saved 3D data
        with open(args.pred_save_path_3d, "w") as f:
            json.dump(
                dict(
        ...
        print(f"predictions have been saved at {args.pred_save_path_2d}")
